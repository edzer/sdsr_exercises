...
In the vignette, two upper-level variables are added to the six already present.
...
```{r}
spdep::moran.test(depmunic$greensp, nb2listw(mun_nb))
```
...
```{r}
(pr_nb_k4s <- spdep::knn2nb(spdep::knearneigh(properties, k=4), sym=TRUE))
```
...
```{r}
spdep::moran.test(properties_in_dd$greensp, nb2listw(pr_nb_k4s))
```
...
f_pr_md <- update(f_pr, . ~ . + foreigners + greensp + popdens + museums + airbnb)
```
